...
class SimulationRunner:
    ...
    def test_dataset_auto_loading(self):
        """Test that dataset is automatically loaded on startup."""
        test_name = "Dataset auto-loading"
        self.log(f"Testing {test_name}...")
        
        # Test: Just exit - dataset should be auto-loaded
        inputs = [
            "4",  # Main menu: Exit
        ]
        ...
        if success:
            # Check that dataset was loaded (should see "Loaded dataset" message)
            if "Loaded dataset" in stdout or "sales_data.csv" in stdout:
                self.test_results.append((test_name, "PASS"))
                print(f"[✓] {test_name}: PASS")
            else:
                self.test_results.append((test_name, "FAIL - Dataset not auto-loaded"))
                print(f"[✗] {test_name}: FAIL - Dataset not auto-loaded")
        else:
    ...
    def test_correct_answer_display(self):
        ...
        inputs = [
            "2",  # Main menu: Run Exercise (dataset auto-loaded)
    ...
    def test_exit_command(self):
        ...
        for n_rows in range(5, 16):
            inputs = [
                "2",  # Main menu: Run Exercise (dataset auto-loaded)
                "1",  # Exercise menu: Exercise 1
                f"df.head({n_rows})",  # Correct answer for task 1 - try this number
                "exit",  # Exit during task 2
                "",  # Press Enter
                "4",  # Main menu: Exit
            ]
            
            test_success, stdout, stderr = self.run_simulation(
                inputs,
                f"{test_name} (trying n={n_rows})"
            )
            
            # Check if this value worked (look for success indicators)
            if test_success and ("Exercise exited" in stdout or "exited" in stdout.lower() or "Correct" in stdout or "TASK 2" in stdout):
                success = True
                break
    ...
    def run_all_tests(self):
        ...
        # Run tests
        self.test_dataset_auto_loading()
