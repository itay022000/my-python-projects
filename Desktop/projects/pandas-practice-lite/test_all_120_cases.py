#!/usr/bin/env python3
"""
Comprehensive test to verify all 40 cases:
- 5 exercises × 8 tasks × 1 dataset = 40 test cases
Ensures correct answers are accepted for every task in every exercise with the dataset.
"""
...
class Comprehensive120TestRunner:
    def __init__(self):
        self.passed = defaultdict(int)  # exercise -> count
        self.failed = defaultdict(list)  # exercise -> list of failed tasks
        self.total_tests = 0
        self.results = {}  # (exercise, task) -> (passed, reason)
    ...
    def test_single_task(self, exercise_num, task_num):
        ...
        success, reason, stdout, stderr = self.run_simulation(
            inputs,
            f"Exercise {exercise_num}, Task {task_num}",
            timeout=300
        )
        
        self.total_tests += 1
        key = (exercise_num, task_num)
    ...
    def run_all_tests(self):
        ...
        print("\n" + "="*70)
        print("COMPREHENSIVE TEST: All 40 Cases")
        print("5 exercises × 8 tasks × 1 dataset = 40 test cases")
        print("="*70)
        print("\nThis will test that correct answers are accepted for:")
        print("  - Every task (1-8) in every exercise (1-5)")
        print("  - With the dataset (sales_data.csv)")
        print("="*70)
        
        exercises = [1, 2, 3, 4, 5]
        tasks = [1, 2, 3, 4, 5, 6, 7, 8]
        
        total_cases = len(exercises) * len(tasks)
        current_case = 0
        ...
        for exercise_num in exercises:
            ...
            for task_num in tasks:
                ...
                passed = self.test_single_task(exercise_num, task_num)
                
                if passed:
                    self.passed[exercise_num] += 1
                    status = "PASS"
                else:
                    self.failed[exercise_num].append(task_num)
                    status = "FAIL"
                
                result = self.results.get((exercise_num, task_num), (False, "Unknown"))
        ...
        # Detailed breakdown
        print(f"\n{'='*70}")
        print("DETAILED BREAKDOWN BY EXERCISE")
        print(f"{'='*70}")
        
        for exercise_num in exercises:
            passed_count = self.passed.get(exercise_num, 0)
            failed_tasks = self.failed.get(exercise_num, [])
            
            if failed_tasks:
                print(f"  Exercise {exercise_num}: {passed_count}/8 tasks passed")
                print(f"    Failed tasks: {failed_tasks}")
            else:
                print(f"  Exercise {exercise_num}: ✓ All 8 tasks passed")
        ...
        if total_failed > 0:
            ...
            for key, result in self.results.items():
                if not result[0]:
                    exercise_num, task_num = key
                    print(f"  Exercise {exercise_num}, Task {task_num}: {result[1]}")
        
        print("\n" + "="*70)
        if total_failed == 0:
            print("✓ ALL 40 TEST CASES PASSED!")
        else:
