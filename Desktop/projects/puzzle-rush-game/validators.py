...
Used only for coding challenges. True/false answers are exact ``True`` / ``False``
string compares in ``engine`` (same rule as python-basics batch 2).
"""

from __future__ import annotations

# Matrix transpose coding challenges accept only these two spellings (after whitespace normalize).
_MATRIX_TRANSPOSE_FORMS = frozenset({"M.T", "np.transpose(M)"})
...
def validate_code_answer(user_input: str, correct_answer: str, profile: str = "default") -> bool:
    ...
    Profiles:
      ...
      - matrix: tuple-shape whitespace rules; transpose challenges accept only
        ``M.T`` or ``np.transpose(M)`` (mutually interchangeable).
    """
    ...
    if profile == "matrix":
        ...
        # Transpose coding challenges: only M.T and np.transpose(M) are valid.
