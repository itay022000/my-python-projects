...
from validators import checker_mixed_functions
...
class Batch8Exercises:
    ...
    def start_exercises(self):
        ...
        print_batch_intro(
            "Functions Exercise",
            "You will get 11 questions (~9.09% each). One asks for two lines; the rest ask for one.",
            background=EXERCISE_BACKGROUNDS[8],
        )
        ...
        try:
            units_passed, units_failed = run_mixed_units_session(
                ...
                hint_for=lambda ex: resolve_hint(8, ex),
            )
        except SessionExit:
            return
